...
public:
   ...
   //receiving only a logData in case of a read operation
    void sendSerialCommand(commandType command, uint8_t controllerID, uint8_t registerNumber, uint8_t value); 
    logData receiveSerialLog(uint8_t controllerID); // receive a log from the serial port and return it as a vector of bytes (logData
    std::vector<logData> receiveSerialLogs(uint8_t controllerID); // receive all logs from the serial port and return them as a vector of bytes
